...
void HookFrameGen(/*HANDLE hProcess, uintptr_t moduleBaseAddress*/) {
    ...
    */
    ...
    int address = g_cheatdata->address->FrameGen;
    int offset = g_cheatdata->offset->FrameGen;
    int value = g_cheatdata->FxxkFrameGen ? 1 : 0;

    /* 被开发者搞砸
    if (MM::GetInt(address, 0x0) == 0) { // 0x0 是FrameGen的UI而不是FrameGen本身，而前者正是受飞行错误影响的东西，虽然前者能调用后者
        return; // Frame Gen UI Disabled!
    }
    */

    // 如果通过命令关闭帧生成，我们直接设置FrameGen为0然后返回
    if (!g_cheatdata->FxxkFrameGen) {
        MM::SetInt(address, offset, value);
        return;
    }

    // 如果开启，我们始终将FrameGen设置为1
...
void HookRenderResolution(/*HANDLE hProcess, uintptr_t moduleBaseAddress*/) {
    ...
    return; // 现在不需要它了，我们直接在 Command 进行操作
    ...
    */
    ...
    int address = g_cheatdata->address->RenderResolution;
    int offset = g_cheatdata->offset->RenderResolution;
    float value = 75.f;
