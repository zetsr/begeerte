...
void c_FrameGen(const ConsoleEvent& e) {
    ...
    if (std::regex_match(e.text, command_regex)) {
        ...
        if (int_params.size() == 1) {
            ...
            g_cheatdata->FxxkFrameGen = value;
        }
        else {
            print("参数数量不正确");
        }
...
void c_RenderResolution(const ConsoleEvent& e) {
    // 修改正则表达式以接受任意数字
    std::regex command_regex("^set\\s+RR\\s+(\\d+(\\.\\d+)?)$", std::regex_constants::icase);

    if (std::regex_match(e.text, command_regex)) {
        // 提取数字参数
        std::vector<float> num_params = extract_float_params(e.text, 1); // 假设extract_double_params可以提取浮动数字
